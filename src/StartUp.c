...
extern ULONG __saveds __stdargs L_OpenLibs(struct LibraryBase *LibraryBase);
extern void  __saveds __stdargs L_CloseLibs(void);

struct LibraryBase * __saveds ASM InitLib( register __a6 struct ExecBase    *sysbase GNUCREG(a6),
                                           register __a0 SEGLISTPTR          seglist GNUCREG(a0),
                                           register __d0 struct LibraryBase *exb     GNUCREG(d0));
struct LibraryBase * __saveds ASM OpenLib( register __a6 struct LibraryBase *LibraryBase GNUCREG(a6));
SEGLISTPTR __saveds ASM CloseLib( register __a6 struct LibraryBase *LibraryBase GNUCREG(a6));
SEGLISTPTR __saveds ASM ExpungeLib( register __a6 struct LibraryBase *exb GNUCREG(a6));
...
{
 (ULONG)               sizeof(struct LibraryBase),
 (APTR              *) &FuncTab[0],
...
APTR FuncTab [] =
{
    ...
    (APTR) _C2P_CopyMem,
    (APTR) _C2P_FreeMem,
    (APTR) _C2P_CreateContext,
    (APTR) _C2P_InitializeContext,
    (APTR) _C2P_DestroyContext,
    (APTR) _C2P_GetContextParameter,
    (APTR) _C2P_SetContextParameter,
    (APTR) _C2P_GetContextIndexedParameter,
    (APTR) _C2P_SetContextIndexedParameter,
    (APTR) _C2P_Chunky2Planar,
    (APTR) _C2P_WritePixel,
    (APTR) _C2P_GetSystemParameter,
    (APTR) _C2P_GetSystemIndexedParameter,
    (APTR) _C2P_CreateBitMap,
    (APTR) _C2P_DestroyBitMap,
                                           ...
                                           register __d0 struct LibraryBase   *exb     GNUCREG(d0))
{
    ...
    LibraryBase->lb_SysBase = sysbase;
    LibraryBase->lb_SegList = seglist;

    if(L_OpenLibs(LibraryBase)) return(LibraryBase);

    L_CloseLibs();

    {
        ULONG negsize, possize, fullsize;
        UBYTE *negptr = (UBYTE *) LibraryBase;

        negsize  = LibraryBase->lb_LibNode.lib_NegSize;
        possize  = LibraryBase->lb_LibNode.lib_PosSize;
...
struct LibraryBase * __saveds ASM OpenLib( register __a6 struct LibraryBase *LibraryBase GNUCREG(a6))
{
    ...
    LibraryBase->lb_LibNode.lib_OpenCnt++;

    LibraryBase->lb_LibNode.lib_Flags &= ~LIBF_DELEXP;
...
SEGLISTPTR __saveds ASM CloseLib( register __a6 struct LibraryBase *LibraryBase GNUCREG(a6))
{
    ...
    if(!LibraryBase->lb_LibNode.lib_OpenCnt)
    {
        if(LibraryBase->lb_LibNode.lib_Flags & LIBF_DELEXP)
        {
            return( ExpungeLib(LibraryBase) );
        }
    }
...
SEGLISTPTR __saveds ASM ExpungeLib( register __a6 struct LibraryBase *exb GNUCREG(a6))
{
    struct LibraryBase *LibraryBase = exb;
    SEGLISTPTR seglist;
    ...
    if(!LibraryBase->lb_LibNode.lib_OpenCnt)
    {
        ...
        seglist = LibraryBase->lb_SegList;
