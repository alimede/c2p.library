...
VOID CleanUp(VOID);
VOID FillChunkyData(UBYTE *chunky, ULONG displacement, LONG offsets_count, LONG *offsets);
...
int main()
{
    ...
    // force use of scrambled chunky mode
    C2P_SetContextParameter(Context, C2P_CONTEXT_PARAMETER_FORCE_SCRAMBLED, TRUE);
    ...
    LONG *scrambled_offsets = C2P_AllocMem(Context, scrambled_offsets_count * sizeof(LONG));
    printf("Scrambled offsets: 0x%08x\n", scrambled_offsets);
    ...
    if (scr != NULL)
    {
        ...
        if (win != NULL)
        {
            ...
            {
                FillChunkyData(chk, i, scrambled_offsets_count, scrambled_offsets);
...
VOID FillChunkyData(UBYTE *chunky, ULONG displacement, LONG offsets_count, LONG *offsets)
{
    ULONG mask = offsets_count - 1;

    for (ULONG x = 0; x < WIDTH ; x++)
    {
        LONG ofs = offsets[x & mask];

        UBYTE *row = chunky;

        for (ULONG y = 0; y < HEIGHT ; y++)
        {
            row[x + ofs] = (x + displacement) & 0xFF;
            row += WIDTH;
        }
    }
