...
int main()
{
    ...
    BitMap = C2P_CreateBitMap(Context, WIDTH, HEIGHT, C2P_CONTEXT_PLANAR_FORMAT_8_BIT);
    if (BitMap == NULL)
    {
    ...
    printf("Custom BitmMap: 0x%08x\n", BitMap);
    for (int i = 0; i < 8; i++)
        printf("Bitplane %d address: 0x%08x\n", i, BitMap->Planes[i]);

    C2P_SetContextParameter(Context, C2P_CONTEXT_PARAMETER_TYPE, C2P_CONTEXT_TYPE_CUSTOM_BITMAP);
    C2P_SetContextParameter(Context, C2P_CONTEXT_PARAMETER_BITMAP, (ULONG)BitMap);
...
BOOL Initialize(VOID)
{
    ...
    do
    {
        ...
        ULONG cpu = C2P_GetSystemParameter(C2P_SYSTEM_PARAMETER_CPU);
        switch (cpu)
        {
